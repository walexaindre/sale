<script lang="ts">
  ...
  import * as parser from "./parserold";
  import DetailedItem from "./lib/DetailedItem.svelte";

  let system = new parser.SystemHandler();
  system.generateRandomSystemData();
...
<div class="navbar text-neutral-content z-20 sticky top-0 bg-base-100">
  ...
  <div class="navbar-center">
    <label class="input">
      ...
      <input type="text" class="text-neutral" placeholder="Type something" />
    </label>
  </div>
...
<div class="grid grid-cols-[repeat(auto-fill,minmax(384px,1fr))] gap-4 p-4">
  {#each system.getAllItems() as item}
    <Item
      name={item.name}
      ...
      imgdesc={""}
      price={item.price}
      quantity={system.getLeftQuantity(item.itemId)}
      tags={item.tags}
